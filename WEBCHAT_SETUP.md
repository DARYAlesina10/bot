...
- `webchat_api.php` — API для виджета:
  ...
  - поддерживает отправку изображений от посетителя
  - `action=poll` — отдаёт ответы оператора из Telegram обратно в виджет
  - `action=history` — отдаёт всю историю сессии (включая сообщения посетителя)
...
- Обработка ответов менеджера из Telegram в `bot.php`:
  ...
  - фото менеджера отправляются в виджет как картинка
...
5. В нерабочее время (18:00–10:00 UTC) посетителю автоматически приходит автоответ.
...
- Веб-чат поддерживает текст + картинки от посетителя и оператора.
- Если нужно другое расписание автоответа, поменяйте `wcShouldSendOffHoursAutoReply()` в `webchat_api.php`.
